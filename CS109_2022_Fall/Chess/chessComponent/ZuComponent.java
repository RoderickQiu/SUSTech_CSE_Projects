...
public class ZuComponent extends ChessComponent {

    public ZuComponent(ChessboardPoint chessboardPoint, Point location, ChessColor chessColor, ClickController clickController, int size) {
    ...
    public boolean canMoveTo(SquareComponent[][] chessboard, ChessboardPoint destination, ChessboardPoint ONE, ChessColor playerColor) {
        ...
        if (destinationChess.getChessColor() == playerColor) {//red 为自己的棋子
            ...
            if (destinationChess.getCategory() <= 1 || destinationChess.getCategory() == 7) {//扣除敌方血量
                return true;
            } else {
